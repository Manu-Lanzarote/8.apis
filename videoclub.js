...
// Nos han pedido hacer una web para el videoclub del barrio. Tenemos que mostrar en el HTML todas las películas que hay en el catálogo (objeto videoclub). Para ello, cada película irá en una carta/tarjeta: tendrá un borde definido y con todos los datos de cada película. Además, en cada carta/tarjeta pondremos un botón de “Alquilar” que, cuando lo pulsemos, mostraremos en un alert el mensaje “Has alquilado la película”.
let peli = "";

for (let i = 0; i < videoclub.peliculas.length; i++) {
  //   console.log(videoclub.peliculas[i]);
  peli += `<div class="card">
  <img src="${videoclub.peliculas[i].cartel}" alt="${videoclub.peliculas[i].titulo}" style="width:100%">
  <div class="container">
    <h1><b>${videoclub.peliculas[i].titulo}</b></h1> 
    <p>${videoclub.peliculas[i].sinopsis}</p> 
  </div>
  <button onclick="alerta()" id="alquilar">ALQUILAR</button>
</div>
  `;
  document.getElementById("cartelera").innerHTML = peli;
}
function alerta() {
  alert("Has alquilado la película");
}
